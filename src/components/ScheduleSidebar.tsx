...
export const ScheduleSidebar = ({
  ...
  const renderMenuItem = (item: typeof filterMenuItems[0], index: number) => {
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        <nav className="flex-1 p-2">
          {!isCollapsed && (
            <p className="px-3 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Schedules</p>
          )}
          <div className="space-y-0.5">
            {filterMenuItems.map((item, index) => renderMenuItem(item, index))}
          </div>
        </nav>
      </motion.aside>
    </TooltipProvider>
